    ...
    return(
        <>
            <div className='flex gap-4 w-full h-full relative'>
                ...
                <div className="w-1/2 rounded-md p-4 bg-white h-full">
                    <LivePreview postTitle={debouncedTitle ?? 'New Post Title'} postBodyDebounced={debouncedBody ?? 'New Post Body' } />
                </div>             
            </div>
        </>
    )
    ...
    return (
        <>
            <div className="flex flex-col gap-4 w-full h-full">
                <input className="border-b-[1.5px] border-blue p-2 text-xl w-fit min-w-[450px] max-w-[100%] focus:bg-slate-50 focus:outline-2 focus:outline-slate-200 focus:ring-0" placeholder="Post Title" onChange={(e) => {changeTitle(e.target.value); console.log('hi')}} type="text" required />
                <textarea className="border-b-[1.5px] border-blue p-2 text-xl w-full h-full focus:bg-slate-50 focus:outline-2 focus:outline-slate-200 focus:ring-0"  placeholder='Post Body' onChange={(e) => {changeBody(e.target.value)}} />
            </div>
        </>
    )
    ...
    return (
        <>
            <div className="flex flex-col gap-4" >
                <h1 className={`text-3xl font-semibold ${previewStyles.title}`}>{postTitle}</h1>
                <div className={`preview w-full overflow-x-hidden max-w-[calc(100%-70px)] overflow-y-auto ${previewStyles.body}`}>{parse(postBodyHtml.toString())}</div>
            </div>
        </>
    )
}
